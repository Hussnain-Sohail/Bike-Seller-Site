...
async function Signup(req, res) {
    try {
        ...
        await newUser.save();
        const AccessToken = jwt.sign({ userId: newUser._id }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: '60s' });
        const RefreshToken = jwt.sign({ userId: newUser._id }, process.env.Refresh_TOKEN_SECRET, { expiresIn: '5min' });
